...
4. Điền tên Database (ví dụ: `htxrau-database`) và cấu hình (để mặc định).
5. Click **Deploy**.
...
2. Dán link GitHub repository của bạn (ví dụ: `https://github.com/hclhcl0/htxrau`).
3. Chọn nhánh `main` hoặc nhánh bạn muốn deploy.
...
| `NEXT_PUBLIC_SERVER_URL` | `https://rautuyloan.com` | Tên miền của bạn được cấu hình trên Coolify. |
...
2. Thêm một Volume mới:
   ...
   * **Volume Name**: đặt tên gợi nhớ, ví dụ: `htxrau-media-volume`.
...
2. Điền tên miền của bạn (ví dụ: `https://rautuyloan.com` hoặc `https://test.yourdomain.com`).
